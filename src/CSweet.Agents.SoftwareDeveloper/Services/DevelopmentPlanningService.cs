...
internal sealed class DevelopmentPlanningService(
    ...
    DevelopmentChatClientProvider chatClients)
{
    ...
    internal async Task<CreatePersonalWorkPlanRequest> PlanAsync(PersonalTodoItem item, string requestText,
...
""", text =>
                {
                    var outline = JsonSerializer.Deserialize<DevelopmentPlanOutline>(DevelopmentJson.StripFence(text), _serializerOptions)
...
""", text =>
                {
                    var result = JsonSerializer.Deserialize<DevelopmentStoryTasks>(DevelopmentJson.StripFence(text), _serializerOptions)
        ...
        async Task<DevelopmentPlanDraft> GenerateAsync(string progress, string stage, Func<string, DevelopmentPlanDraft> parse)
        {
            ...
            {
                new(ChatRole.System, instructions), new(ChatRole.User, requestText), new(ChatRole.User, stage)
            };
            for (var attempt = 0; attempt < 3; attempt++)
            {
                var response = await client.GetResponseAsync(messages, new ChatOptions
                {
                    MaxOutputTokens = Math.Clamp(_settings.GetInt32("maxOutputTokens", SoftwareDeveloperHarness.DefaultOutputTokens),
    ...
    internal static string TaskScope(PersonalTodoItem? task, string wholeRequest) => task is null ? wholeRequest :
