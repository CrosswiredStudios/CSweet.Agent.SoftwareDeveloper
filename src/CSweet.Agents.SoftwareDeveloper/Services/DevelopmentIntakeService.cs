...
internal sealed class DevelopmentIntakeService
{
    private static readonly JsonSerializerOptions _serializerOptions = new(JsonSerializerDefaults.Web);
    private readonly AgentSettings _settings;
    private readonly DevelopmentChatClientProvider _chatClients;
    private readonly ProjectIntakeService _projects;

    internal DevelopmentIntakeService(AgentSettings settings, DevelopmentChatClientProvider chatClients)
    {
        _settings = settings;
        _chatClients = chatClients;
        _projects = new ProjectIntakeService(chatClients);
    }
    ...
    internal async Task HandleDirectWorkMessageAsync(Guid chatId, CommunicationMessage source,
        ...
        AgentRuntimeContext context, CancellationToken ct)
    {
        ...
        if (await _projects.HandleProjectChoiceAsync(chatId, source, received, context, ct)) return;
        ...
        if (intake is null || source.Sequence > intake.Sequence)
        {
            ...
            retained = await DevelopmentStateStore.SaveAsync(key, intake, retained, source.Id, context, ct);
        ...
        await ReplyAsync(ProjectIntakeService.ProjectSetupReply(projectIntake));
        ...
        var response = await client.GetResponseAsync([
            ...
            new(ChatRole.User, JsonSerializer.Serialize(new { currentHumanMessage = message, pending,
                ...
                recentHistory = history.OrderByDescending(x => x.Sequence).Take(12).OrderBy(x => x.Sequence)
                    .Select(x => new { x.SenderDisplayName, x.SenderEmployeeType, x.Content }) }, _serializerOptions))],
            new ChatOptions { MaxOutputTokens = _settings.GetInt32("maxOutputTokens", SoftwareDeveloperHarness.DefaultOutputTokens) }, ct);
        return JsonSerializer.Deserialize<IntakeDecision>(DevelopmentJson.StripFence(response.Text), _serializerOptions) ?? throw new JsonException("Empty work intake.");
    }

    internal static bool IsDirectWork(PersonalTodoItem item) => item.Description.Contains(DevelopmentStateStore.DirectWorkMarker, StringComparison.Ordinal);
}
