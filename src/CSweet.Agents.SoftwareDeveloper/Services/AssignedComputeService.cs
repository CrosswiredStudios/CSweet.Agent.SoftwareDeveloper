...
internal sealed class AssignedComputeService
{
    private readonly AgentSettings _settings;

    internal AssignedComputeService(AgentSettings settings) => _settings = settings;
    ...
    internal sealed record AssignedComputeReadiness(bool Ready, ComputeEnvironment? Environment, string? FailureCode);

    internal async Task<AssignedComputeReadiness> EnsureAsync(
        AgentRuntimeContext context, CancellationToken cancellationToken, bool notifyManager = true, Guid? projectId = null)
    {
        ...
        if (state.EnvironmentId is { } environmentId)
        {
            ...
            if (environment.State is "failed" or "destroyed" || environment.LeaseExpiresAt <= DateTimeOffset.UtcNow)
            {
                ...
                if (state.ReplacementGeneration >= _settings.GetInt32("maximumComputeReplacements", 3))
        ...
        environment = await context.Platform.Compute.ProvisionAsync(new(
            ...
            new ComputeSpecification("linux", "x64", templateId, new(2, 2048, 20480),
                _settings.GetInt32("computeLifetimeSeconds", 0))), cancellationToken);
    ...
    private async Task<AssignedComputeReadiness> EnsureProjectComputeAsync(Guid project, AgentRuntimeContext context, CancellationToken ct)
    {
        ...
        if (state.EnvironmentId is { } id)
        {
            ...
            if (state.ReplacementGeneration >= _settings.GetInt32("maximumComputeReplacements", 3)) return new(false, current, "replacement-limit-reached");
            state = state with { EnvironmentId = null, ReplacementGeneration = state.ReplacementGeneration + 1 };
        }
        ...
        var environment = await context.Platform.Compute.ProvisionAsync(new(project, desired, desired,
            new ComputeSpecification("linux", "x64", template, new(2, 2048, 20480), _settings.GetInt32("computeLifetimeSeconds", 0))), ct);
