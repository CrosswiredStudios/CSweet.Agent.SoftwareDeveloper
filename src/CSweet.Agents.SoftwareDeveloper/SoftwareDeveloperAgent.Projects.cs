...
public sealed partial class SoftwareDeveloperAgent
{
    ...
    private static string ProjectSetupReply(ProjectIntakeSummary intake) => intake.Status == "AwaitingAssignment"
    ...
    private async Task<bool> HandleProjectChoiceAsync(Guid chatId, CommunicationMessage source, CommunicationMessageReceivedEvent? received, AgentRuntimeContext context, CancellationToken ct)
    {
...
"""), new(ChatRole.User, JsonSerializer.Serialize(new { currentHumanMessage = source.Content, intake, accessibleProjects = candidates }, SerializerOptions))],
            new ChatOptions { MaxOutputTokens = 1000 }, ct);
        var choice = JsonSerializer.Deserialize<ProjectChoice>(StripJsonFence(response.Text), SerializerOptions) ?? throw new JsonException("Missing project choice.");
    ...
    private async Task ResumeProjectIntakeAsync(ProjectIntakeSummary intake, AgentRuntimeContext context, CancellationToken ct)
    {
